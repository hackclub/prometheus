  ...
  const blocks = [
    {
      ...
      text: {
        type: "mrkdwn",
        text: `:wastebasket: <@${deletedBy}> deleted a message from <@${sender}> in <#${channel}> <!date^${Math.floor(parseFloat(ts))}^{date_short_pretty} {time_secs}|${ts}>`,
      },
    },
  ];
...
export async function logMove(client, { source, dest, movedBy, result }) {
  if (!LOG_CHANNEL) return;

  const skipped = result.skipped;
  const skipParts = [];
  if (skipped.excluded.length) skipParts.push(`${skipped.excluded.length} excluded`);
  if (skipped.banned.length) skipParts.push(`${skipped.banned.length} banned from dest`);
  if (skipped.optedOut.length) skipParts.push(`${skipped.optedOut.length} opted out`);

  const lines = [
    `<@${movedBy}> moved members from <#${source}> to <#${dest}>`,
    `> *${result.invited.length}* invited - ${result.alreadyIn.length} already in - ${result.sourceCount} in source`,
  ];
  if (skipParts.length) lines.push(`> _Skipped: ${skipParts.join(", ")}_`);
  ...
  await getLogClient(client).chat.postMessage({
    channel: LOG_CHANNEL,
    text: `Members moved from <#${source}> to <#${dest}>`,
