...
const DELETED_TEXT_LABEL = "*Here's what you sent:*\n";
const SECTION_TEXT_LIMIT = 3000;
const TRUNCATION_SUFFIX = "\n> … (truncated)";

function buildDeletedTextBlockText(deletedText) {
  const quotedText = quoted(deletedText);
  const budget = SECTION_TEXT_LIMIT - DELETED_TEXT_LABEL.length - TRUNCATION_SUFFIX.length;
  let truncatedText = quotedText.slice(0, budget);
  if (/[\uD800-\uDBFF]$/.test(truncatedText)) truncatedText = truncatedText.slice(0, -1);
  const body = quotedText.length > budget ? `${truncatedText}${TRUNCATION_SUFFIX}` : quotedText;
  return `${DELETED_TEXT_LABEL}${body}`;
}

export function buildPostingGateMessage(
  gate,
  { retry = false, stale = false, welcome, deletedText } = {},
) {
  const blocks = [];
  ...
  if (retry && deletedText) {
    blocks.push({
      type: "section",
      text: { type: "mrkdwn", text: buildDeletedTextBlockText(deletedText) },
    });
  }
