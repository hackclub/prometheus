import { getAnchorPollById, getAnchorNpsResponses, recordNpsScore } from "../db.js";
import { buildAnchorNpsBlocks, isNpsSurveyClosed } from "../blocks/anchorNps.js";
import { syncNpsResponse } from "../airtable.js";

const ACTION_ID_PATTERN = /^anchor_nps_score_(\d{1,2})$/;

export default {
  actionId: ACTION_ID_PATTERN,

  async execute({ ack, body, action, client, context, logger }) {
    await ack();

    const score = parseInt(action.action_id.match(ACTION_ID_PATTERN)[1], 10);
    const { poll: pollId } = JSON.parse(action.value);
    const userId = body.user.id;

    const poll = await getAnchorPollById(pollId);
    if (!poll || poll.type !== "nps" || !poll.enabled) return;

    if (!poll.is_current || isNpsSurveyClosed(poll)) {
      try {
        await client.chat.postEphemeral({
          channel: poll.channel_id,
          user: userId,
          text: "This survey has closed.",
        });
      } catch {
        /* best effort */
      }
      return;
    }

    await recordNpsScore(poll.id, userId, score);

    try {
      await syncNpsResponse(poll, userId, { score }, client, context);
    } catch (e) {
      logger.warn(`anchor nps score airtable sync failed in ${poll.channel_id}: ${e.message}`);
    }

    if (poll.message_ts) {
      try {
        await client.chat.update({
          channel: poll.channel_id,
          ts: poll.message_ts,
          text: poll.question,
          blocks: buildAnchorNpsBlocks(poll, await getAnchorNpsResponses(poll.id)),
        });
      } catch (e) {
        logger.error(`anchor nps score re-render failed in ${poll.channel_id}: ${e.message}`);
      }

      try {
        await client.chat.postEphemeral({
          channel: poll.channel_id,
          user: userId,
          thread_ts: poll.message_ts,
          text: `Thanks! Recorded your score: *${score}*. Feel free to reply in this thread with any feedback.`,
        });
      } catch {
        /* best effort */
      }
    }
  },
};
