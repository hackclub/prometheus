...
async function pollDetailBlocks(poll) {
...
async function npsDetailBlocks(poll) {
  const responses = await getAnchorNpsResponses(poll.id);
  if (!responses.length)
    return [{ type: "section", text: { type: "mrkdwn", text: "_No responses yet._" } }];
      ...
      await client.views.open({
        ...
        view: {
          ...
          blocks: blocks.length
            ? blocks
            : [{ type: "section", text: { type: "mrkdwn", text: "_No data._" } }],
        },
      });
    } catch (e) {
