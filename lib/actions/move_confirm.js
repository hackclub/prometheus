...
export default {
  ...
  async execute({ ack, body, action, respond, client, context, logger }) {
    ...
    const canSource = await canManage(context.userClient, movedBy, source);
    const canDest = await canManage(context.userClient, movedBy, dest);
    if (!canSource || !canDest) {
      return respond({
        replace_original: true,
        text: ":loll: You no longer manage both channels.",
      });
    }
