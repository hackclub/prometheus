import { listConfiguredChannelIds, listUserAppointedManagers } from "../db.js";
...
let catalogCache = { expiresAt: 0, promise: null };
...
async function fetchChannelCatalog(client) {
  ...
  do {
    ...
    for (const channel of response.channels || []) channels.set(channel.id, channel);
    cursor = response.response_metadata?.next_cursor;
  } while (cursor);
  return channels;
}

function loadChannelCatalog(client) {
  if (catalogCache.promise && catalogCache.expiresAt > Date.now()) return catalogCache.promise;

  const promise = fetchChannelCatalog(client);
  catalogCache = { expiresAt: Date.now() + CHANNEL_CACHE_TTL_MS, promise };
  promise.catch(() => {
    if (catalogCache.promise === promise) catalogCache = { expiresAt: 0, promise: null };
  });
  return promise;
}
...
export async function loadPermissions(client, userId, discoveryClient = client) {
  ...
  if (globalAdmin) {
    for (const channelId of await listConfiguredChannelIds()) channelIds.add(channelId);
  }
  const catalog = await loadChannelCatalog(discoveryClient);
  const visibleChannels = (
    await Promise.all(
      [...channelIds].map(async (channelId) => {
        const grant = grants.get(channelId);
        let channel = catalog.get(channelId);
        if (!channel && grant) channel = await channelInfo(channelId, [client, discoveryClient]);
        if (!channel && !grant && !globalAdmin) return null;
