...
import { createDisclosureRouter } from "./disclose.js";
...
const CREATED_KEY_COOKIE = "prom_created_key";
const CREATED_KEY_SHAPE = /^prom_[A-Za-z0-9_-]{20,80}$/;
...
function createdKeyCookieOptions(c, channelId, extras = {}) {
  return {
    httpOnly: true,
    path: sectionPath(channelId, ""),
    sameSite: "Lax",
...
function flashCreatedKey(c, channelId, key) {
  setCookie(c, CREATED_KEY_COOKIE, key, createdKeyCookieOptions(c, channelId, { maxAge: 60 }));
}

function takeCreatedKey(c, channelId) {
  const key = getCookie(c, CREATED_KEY_COOKIE);
  if (!key) return null;
  deleteCookie(c, CREATED_KEY_COOKIE, createdKeyCookieOptions(c, channelId));
  return CREATED_KEY_SHAPE.test(key) ? key : null;
}
...
function requestIsSameOrigin(c) {
  ...
  // Navigational form POSTs under Referrer-Policy: no-referrer send Origin: null.
  // Sec-Fetch-Site is browser-controlled and still rejects cross-site CSRF.
  return (origin === "null" || !origin) && c.req.header("Sec-Fetch-Site") === "same-origin";
}
...
export function createWebApp({ client, botClient = client, isHealthy = () => true }) {
  ...
  app.route("/api/keys", createDisclosureRouter({ botClient, client }));

  app.route("/api/public/v1", createAppointmentsRouter());
  ...
  async function renderSection(c, session, channelId, slug, extra = {}) {
    ...
    const [permissions, teamId] = await Promise.all([
      loadPermissions(client, session.user.slackUserId, botClient),
  ...
  app.get("/c/:channelId/:section?", async (c) => {
    ...
    return renderSection(c, session, channelId, slug, {
      createdKey: slug ? null : takeCreatedKey(c, channelId),
    });
  });
  ...
  async function createKey(c) {
    ...
    try {
      ...
      flashCreatedKey(c, auth.channelId, key);
      return sectionRedirect(c, auth.channelId, "");
    } catch (error) {
  ...
  function redirectLegacyKeys(c) {
    const rawChannelId = c.req.param("channelId");
    if (!CHANNEL_ID.test(rawChannelId)) return c.redirect("/", 303);
    return sectionRedirect(c, rawChannelId.toUpperCase(), "");
  }

  app.post("/c/:channelId", createKey);
  app.post("/c/:channelId/keys/revoke", revokeKey);
  app.post("/channels/:channelId/keys", createKey);
  app.post("/channels/:channelId/keys/revoke", revokeKey);
  app.get("/channels/:channelId/keys", redirectLegacyKeys);
