...
async function upsertByKey(table, keyField, keyValue, fields, createOnlyFields = {}) {
  ...
  const [record] = await base(table).create([{ fields: { [keyField]: keyValue, ...createOnlyFields, ...fields } }]);
...
export async function syncNpsSurvey(poll, client) {
  ...
  const { id } = await upsertByKey(SURVEYS_TABLE, 'Survey Key', surveyKey, {
    ...
    'Closed At': poll.closes_at ? new Date(poll.closes_at * 1000).toISOString() : null,
  });

  return id;
}

export async function syncNpsResponse(poll, userId, patch, client) {
  if (!base) return;
  ...
  await upsertByKey(RESPONSES_TABLE, 'Response Key', `${poll.id}-${userId}`, fields, { 'Submitted At': now });
}
