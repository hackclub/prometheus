...
export async function getPostingGate(channelId) {
  return first(
    await sql`
      SELECT channel_id, mode, prompt, phrase, generation, set_by, enabled, kick_on_join, set_at
  ...
  return first(
    await sql`
      SELECT g.channel_id, g.mode, g.prompt, g.phrase, g.generation,
        g.set_by, g.enabled, g.kick_on_join, g.set_at,
        EXISTS (
...
export async function setPostingGate(channelId, mode, prompt, phrase, setBy, kickOnJoin = 0) {
    ...
    const gate = first(
      await tx`
        ...
        ON CONFLICT (channel_id) DO UPDATE SET
          ...
          set_by = EXCLUDED.set_by,
          enabled = 1,
          kick_on_join = EXCLUDED.kick_on_join,
          set_at = ${sql.unsafe(epoch)}
        RETURNING channel_id, mode, prompt, phrase, generation, set_by, enabled,
          kick_on_join, set_at
      `,
    );
...
export async function setPostingGateKickOnJoin(channelId, kickOnJoin) {
  return first(
    await sql`
      UPDATE channel_posting_gates SET kick_on_join = ${kickOnJoin ? 1 : 0}
      WHERE channel_id = ${channelId} AND enabled = 1
      RETURNING channel_id, mode, prompt, phrase, generation, set_by, enabled,
        kick_on_join, set_at
    `,
