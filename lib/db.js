...
export async function listConfiguredChannelIds() {
  const rows = await sql`
    SELECT channel_id FROM appointed_managers
    UNION
    SELECT channel_id FROM join_messages
    UNION
    SELECT channel_id FROM channel_posting_gates
    UNION
    SELECT channel_id FROM embed_blocks
    UNION
    SELECT channel_id FROM anchor_polls
    UNION
    SELECT channel_id FROM channel_api_keys
    UNION
    SELECT channel_id FROM channel_bans
  `;
...
export async function getPostingGate(channelId) {
  return first(
    await sql`
      SELECT channel_id, mode, prompt, phrase, generation, set_by, enabled, set_at
      FROM channel_posting_gates
      WHERE channel_id = ${channelId}
    `,
  );
}

export async function getPostingGateForUser(channelId, userId) {
  return first(
    await sql`
      SELECT g.channel_id, g.mode, g.prompt, g.phrase, g.generation,
        g.set_by, g.enabled, g.set_at,
        EXISTS (
          SELECT 1 FROM channel_posting_gate_acceptances a
          WHERE a.channel_id = g.channel_id AND a.user_id = ${userId}
        ) AS accepted
      FROM channel_posting_gates g
      WHERE g.channel_id = ${channelId} AND g.enabled = 1
    `,
  );
}

export async function setPostingGate(channelId, mode, prompt, phrase, setBy) {
  const generation = randomUUID();
  return sql.begin(async (tx) => {
    await tx`SELECT pg_advisory_xact_lock(hashtextextended(${`posting_gate:${channelId}`}, 0))`;
    const gate = first(
      await tx`
        INSERT INTO channel_posting_gates
          (channel_id, mode, prompt, phrase, generation, set_by)
        ...
        ON CONFLICT (channel_id) DO UPDATE SET
          ...
          enabled = 1,
          set_at = ${sql.unsafe(epoch)}
        RETURNING channel_id, mode, prompt, phrase, generation, set_by, enabled, set_at
      `,
    );
    await tx`DELETE FROM channel_posting_gate_acceptances WHERE channel_id = ${channelId}`;
    return gate;
  });
}

export async function disablePostingGate(channelId) {
  return Boolean(
    first(
      await sql`
        UPDATE channel_posting_gates SET enabled = 0
        WHERE channel_id = ${channelId} AND enabled = 1
...
export async function acceptPostingGate(channelId, userId, gate) {
  return Boolean(
    first(
      await sql`
        INSERT INTO channel_posting_gate_acceptances (channel_id, user_id)
        SELECT channel_id, ${userId}
        FROM channel_posting_gates
        WHERE channel_id = ${channelId}
          AND enabled = 1
          AND generation = ${gate.generation}
        ON CONFLICT (channel_id, user_id) DO UPDATE
          SET accepted_at = channel_posting_gate_acceptances.accepted_at
        RETURNING user_id
      `,
    ),
  );
...
export async function resetPostingGateAcceptance(channelId, userId) {
  const rows = userId
    ? await sql`
        DELETE FROM channel_posting_gate_acceptances
...
export async function countPostingGateAcceptances(channelId) {
  const [row] = await sql`
    SELECT count(*)::int AS count FROM channel_posting_gate_acceptances
    WHERE channel_id = ${channelId}
  `;
  return row.count;
}
...
export async function revokeChannelApiKeyByHash(keyHash) {
  return sql.begin(async (tx) => {
    const existing = first(
      await tx`
        SELECT id, channel_id, user_id, name, key_prefix, revoked_at
        FROM channel_api_keys WHERE key_hash = ${keyHash}
      `,
    );
    if (!existing) return { found: false };
    if (existing.revoked_at != null) {
      return { found: true, alreadyRevoked: true, key: existing };
    }
    await tx`
      UPDATE channel_api_keys SET revoked_at = ${sql.unsafe(epoch)} WHERE id = ${existing.id}
    `;
    return { found: true, alreadyRevoked: false, key: existing };
  });
}

export async function recordTriviaAnswer(userId, correct, points, difficulty, category) {
  await sql`
    INSERT INTO trivia_answers (user_id, correct, points, difficulty, category)
    VALUES (${userId}, ${correct ? 1 : 0}, ${points}, ${difficulty}, ${category})
  `;
}

export async function getTriviaLeaderboard({ since = 0, difficulty, category, limit = 10 } = {}) {
  return sql`
    SELECT user_id,
...
export async function recordTriviaStreak(userId, streak) {
  await sql`
    INSERT INTO trivia_streaks (user_id, best_streak)
    VALUES (${userId}, ${streak})
    ON CONFLICT (user_id) DO UPDATE
      SET best_streak = GREATEST(trivia_streaks.best_streak, ${streak})
  `;
}

export async function getTriviaPersonalStats(userId) {
  const [byDifficulty, byCategory, streakRows] = await Promise.all([
    sql`
      SELECT difficulty, SUM(correct)::int AS correct, COUNT(*)::int AS played,
             SUM(points)::int AS points
      FROM trivia_answers
      WHERE user_id = ${userId}
      GROUP BY difficulty
    `,
    sql`
      SELECT category, SUM(correct)::int AS correct, COUNT(*)::int AS played
      FROM trivia_answers
      WHERE user_id = ${userId}
      GROUP BY category
    `,
    sql`SELECT best_streak FROM trivia_streaks WHERE user_id = ${userId}`,
  ]);

  const overall = byDifficulty.reduce(
    (acc, row) => ({
      correct: acc.correct + row.correct,
      played: acc.played + row.played,
      points: acc.points + row.points,
    }),
    { correct: 0, played: 0, points: 0 },
  );

  return { overall, byDifficulty, byCategory, bestStreak: streakRows[0]?.best_streak ?? 0 };
}
