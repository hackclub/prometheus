...
export async function addAppointedManager(userId, channelId, addedBy, role = "moderator") {
  await addAppointedManagers([userId], [channelId], addedBy, role);
}

export async function addAppointedManagers(userIds, channelIds, addedBy, role = "moderator") {
...
export async function removeAppointedManager(userId, channelId) {
  await removeAppointedManagers([userId], [channelId]);
}

export async function removeAppointedManagers(userIds, channelIds) {
  await sql.begin(async (tx) => {
    for (const userId of userIds) {
      for (const channelId of channelIds) {
        await tx`
...
export async function listAppointedManagersPage(limit, after, role) {
  const afterUser = after?.userId ?? null;
  const afterChannel = after?.channelId ?? null;
  return sql`
    SELECT user_id, channel_id, role
    FROM appointed_managers
    WHERE (${role}::text IS NULL OR role = ${role})
      AND (${afterUser}::text IS NULL
        OR (user_id, channel_id) > (${afterUser}, ${afterChannel}))
