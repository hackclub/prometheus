...
db.run(`
  CREATE TABLE IF NOT EXISTS user_settings (
    user_id TEXT PRIMARY KEY,
    move_opt_out INTEGER NOT NULL DEFAULT 0,
...
try {
  db.run(`ALTER TABLE anchor_polls ADD COLUMN content TEXT`);
} catch {
  /* column already exists */
}
...
db.run(
  "CREATE UNIQUE INDEX IF NOT EXISTS anchor_polls_channel_current ON anchor_polls (channel_id) WHERE is_current = 1",
);
db.run(
  "CREATE INDEX IF NOT EXISTS anchor_polls_channel_history ON anchor_polls (channel_id, created_at)",
);
...
`);
db.run(
  "CREATE UNIQUE INDEX IF NOT EXISTS anchor_poll_choices_position ON anchor_poll_choices (poll_id, position)",
);
...
`);
db.run(
  "CREATE UNIQUE INDEX IF NOT EXISTS anchor_nps_responses_poll_user ON anchor_nps_responses (poll_id, user_id)",
);
...
{
  const scoreColumn = db
    .query("SELECT \"notnull\" FROM pragma_table_info('anchor_nps_responses') WHERE name = 'score'")
    .get();
  if (scoreColumn?.notnull) {
    ...
    `);
    db.run(
      "INSERT INTO anchor_nps_responses SELECT * FROM anchor_nps_responses_pre_nullable_score",
    );
...
const statements = {
  ...
  addAdmin: db.query(
    "INSERT OR IGNORE INTO global_admins (user_id, added_by) VALUES ($userId, $addedBy)",
  ),
  ...
  hasChannelRole: db.query(
    "SELECT 1 FROM appointed_managers WHERE user_id = $userId AND channel_id = $channelId",
  ),
  isAppointedManager: db.query(
    "SELECT 1 FROM appointed_managers WHERE user_id = $userId AND channel_id = $channelId AND role = $role",
  ),
  addAppointedManager: db.query(
    "INSERT OR REPLACE INTO appointed_managers (user_id, channel_id, added_by, role) VALUES ($userId, $channelId, $addedBy, $role)",
  ),
  removeAppointedManager: db.query(
    "DELETE FROM appointed_managers WHERE user_id = $userId AND channel_id = $channelId",
  ),
  listAppointedManagers: db.query(
    "SELECT user_id, role, added_by, added_at FROM appointed_managers WHERE channel_id = $channelId",
  ),
  listAllAppointedManagers: db.query(
    "SELECT user_id, channel_id, role, added_by, added_at FROM appointed_managers ORDER BY channel_id",
  ),
  hasAppointedManager: db.query("SELECT 1 FROM appointed_managers WHERE channel_id = $channelId"),

  getChannelBan: db.query(
    "SELECT user_id, channel_id, banned_by, reason, expires FROM channel_bans WHERE user_id = $userId AND channel_id = $channelId",
  ),
  setChannelBan: db.query(
    "INSERT OR REPLACE INTO channel_bans (user_id, channel_id, banned_by, reason, expires) VALUES ($userId, $channelId, $bannedBy, $reason, $expires)",
  ),
  removeChannelBan: db.query(
    "DELETE FROM channel_bans WHERE user_id = $userId AND channel_id = $channelId",
  ),
  listChannelBans: db.query(
    "SELECT user_id, channel_id, banned_by, reason, expires FROM channel_bans WHERE channel_id = $channelId",
  ),
  listUserBans: db.query(
    "SELECT user_id, channel_id, banned_by, reason, expires FROM channel_bans WHERE user_id = $userId",
  ),
  listAllChannelBans: db.query(
    "SELECT user_id, channel_id, banned_by, reason, expires FROM channel_bans ORDER BY channel_id",
  ),

  getwelcome: db.query(
    "SELECT channel_id, message, mode, set_by, set_at FROM join_messages WHERE channel_id = $channelId",
  ),
  setwelcome: db.query(
    "INSERT OR REPLACE INTO join_messages (channel_id, message, mode, set_by) VALUES ($channelId, $message, $mode, $setBy)",
  ),
  removewelcome: db.query("DELETE FROM join_messages WHERE channel_id = $channelId"),

  addEmbedBlock: db.query(
    "INSERT OR REPLACE INTO embed_blocks (channel_id, type, target, blocked_by) VALUES ($channelId, $type, $target, $blockedBy)",
  ),
  removeEmbedBlock: db.query(
    "DELETE FROM embed_blocks WHERE channel_id = $channelId AND type = $type AND target = $target",
  ),
  listEmbedBlocks: db.query(
    "SELECT channel_id, type, target, blocked_by, blocked_at FROM embed_blocks WHERE channel_id = $channelId",
  ),
  listAllEmbedBlocks: db.query(
    "SELECT channel_id, type, target, blocked_by, blocked_at FROM embed_blocks ORDER BY channel_id, type, target",
  ),

  getMoveOptOut: db.query("SELECT move_opt_out FROM user_settings WHERE user_id = $userId"),
  setMoveOptOut: db.query(`
    INSERT INTO user_settings (user_id, move_opt_out) VALUES ($userId, $optOut)
    ON CONFLICT (user_id) DO UPDATE SET move_opt_out = excluded.move_opt_out, updated_at = unixepoch()
  `),
  listMoveOptOuts: db.query("SELECT user_id FROM user_settings WHERE move_opt_out = 1"),

  getAnchorPollByChannel: db.query(
    "SELECT * FROM anchor_polls WHERE channel_id = $channelId AND is_current = 1",
  ),
  getAnchorPollById: db.query("SELECT * FROM anchor_polls WHERE id = $id"),
  listAnchorPollsByChannel: db.query(
    "SELECT * FROM anchor_polls WHERE channel_id = $channelId ORDER BY created_at DESC LIMIT $limit",
  ),
  deactivateCurrentAnchorPoll: db.query(
    "UPDATE anchor_polls SET is_current = 0, updated_at = unixepoch() WHERE channel_id = $channelId AND is_current = 1",
  ),
  insertAnchorPoll: db.query(`
    ...
    RETURNING *
  `),
  updateAnchorPollQuestion: db.query(
    "UPDATE anchor_polls SET question = $question, updated_at = unixepoch() WHERE id = $id RETURNING *",
  ),
  setAnchorPollMessageTs: db.query(
    "UPDATE anchor_polls SET message_ts = $ts WHERE channel_id = $channelId AND is_current = 1",
  ),
  setAnchorPollEnabled: db.query(
    "UPDATE anchor_polls SET enabled = $enabled, updated_at = unixepoch() WHERE channel_id = $channelId AND is_current = 1",
  ),

  getAnchorPollChoices: db.query(
    "SELECT * FROM anchor_poll_choices WHERE poll_id = $pollId ORDER BY position ASC",
  ),
  ...
  findAnchorPollVote: db.query(
    "SELECT * FROM anchor_poll_votes WHERE poll_id = $pollId AND choice_id = $choiceId AND user_id = $userId",
  ),
  insertAnchorPollVote: db.query(
    "INSERT INTO anchor_poll_votes (poll_id, choice_id, user_id) VALUES ($pollId, $choiceId, $userId)",
  ),
  deleteAnchorPollVoteById: db.query("DELETE FROM anchor_poll_votes WHERE id = $id"),
  deleteAnchorPollVotesByUser: db.query(
    "DELETE FROM anchor_poll_votes WHERE poll_id = $pollId AND user_id = $userId",
  ),
  ...
  insertAnchorNpsSurvey: db.query(`
    ...
    RETURNING *
  `),
  insertAnchorMessage: db.query(`
    INSERT INTO anchor_polls (channel_id, creator_user_id, question, type, content)
    VALUES ($channelId, $creator, $question, 'message', $content)
    RETURNING *
  `),
  updateAnchorMessageContent: db.query(`
    UPDATE anchor_polls SET question = $question, content = $content, updated_at = unixepoch()
    WHERE id = $id
    RETURNING *
  `),
...
export function isAppointedManager(userId, channelId) {
  return !!statements.isAppointedManager.get({
    $userId: userId,
    $channelId: channelId,
    $role: "manager",
  });
...
export function addAppointedManager(userId, channelId, addedBy, role = "moderator") {
  statements.addAppointedManager.run({
    $userId: userId,
    $channelId: channelId,
    $addedBy: addedBy,
    $role: role,
...
export function isMoveOptedOut(userId) {
  return !!statements.getMoveOptOut.get({ $userId: userId })?.move_opt_out;
}

export function setMoveOptOut(userId, optOut) {
  statements.setMoveOptOut.run({ $userId: userId, $optOut: optOut ? 1 : 0 });
}

export function listMoveOptOuts() {
  return statements.listMoveOptOuts.all().map((row) => row.user_id);
}
...
export const createAnchorPoll = db.transaction(
  (channelId, { creator, question, choices, anonymous, multiSelect, addChoiceSetting }) => {
...
export function toggleAnchorPollVote(pollId, choiceId, userId, single) {
  const existing = statements.findAnchorPollVote.get({
    $pollId: pollId,
    $choiceId: choiceId,
    $userId: userId,
  });
...
export const createAnchorMessage = db.transaction((channelId, { creator, question, content }) => {
  statements.deactivateCurrentAnchorPoll.run({ $channelId: channelId });
  return statements.insertAnchorMessage.get({
    $channelId: channelId,
    $creator: creator,
    $question: question,
...
export function updateAnchorMessageContent(id, { question, content }) {
  return statements.updateAnchorMessageContent.get({
    $id: id,
    $question: question,
    $content: content ?? null,
  });
