...
const statements = {
  ...
  getChannelRole: db.query(
    "SELECT role FROM appointed_managers WHERE user_id = $userId AND channel_id = $channelId",
  ),
...
export function getChannelRole(userId, channelId) {
  return statements.getChannelRole.get({ $userId: userId, $channelId: channelId })?.role ?? null;
}
