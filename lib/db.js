...
const maxConnections = Number.parseInt(process.env.DATABASE_POOL_SIZE || "4", 10);
...
const sql = new SQL({
  ...
  max: Number.isInteger(maxConnections) && maxConnections > 0 ? maxConnections : 4,
  idleTimeout: 30,
...
export async function ensureSchema() {
  await sql`
    CREATE TABLE IF NOT EXISTS global_admins (
      ...
      added_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::bigint)
    )
  `;
  await sql`
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS join_messages (
      ...
      set_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::bigint)
    )
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS embed_blocks (
      ...
      blocked_at BIGINT NOT NULL DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::bigint),
  ...
  `;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS anchor_polls_channel_current
      ON anchor_polls (channel_id) WHERE is_current = 1
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS anchor_polls_channel_history
      ON anchor_polls (channel_id, created_at)
  `;
  await sql`
  ...
  `;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS anchor_poll_choices_position
      ON anchor_poll_choices (poll_id, position)
  `;
  await sql`
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS anchor_poll_votes_poll ON anchor_poll_votes (poll_id)`;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS anchor_poll_votes_unique
      ON anchor_poll_votes (poll_id, choice_id, user_id)
  `;
  await sql`
  ...
  `;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS anchor_nps_responses_poll_user
      ON anchor_nps_responses (poll_id, user_id)
  `;
}
