...
export async function runAnchorNpsCommand({
  ...
  if (days <= 0) {
    return respond(
      eph("Usage: `/pro anchor nps [days] [title]`, days must be a positive whole number."),
    );
  }
  const title =
  ...
  const poll = await createAnchorNpsSurvey(channel, { creator: userId, question: title, days });
  ...
  try {
    ...
    return respond(
      eph(`Created the NPS survey but couldn't post/pin it: \`${e.data?.error ?? e.message}\``),
    );
  }
