...
function contextBlock(poll, { closed } = {}) {
...
export function buildAnchorPollBlocks(poll, choices, votes, { closed = false } = {}) {
  const blocks = [
    titleSection(poll, { closed }),
    { type: "divider" },
    choicesSection(poll, choices, votes),
  ];
  if (!closed) blocks.push(voteActions(poll, choices));
  blocks.push(contextBlock(poll, { closed }));
  return blocks;
}
