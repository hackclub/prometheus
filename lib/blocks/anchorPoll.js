import { resolveDisplayNames } from "../displayNames.js";

const BAR_BLOCKS = [" ", "▏", "▎", "▍", "▌", "▋", "▊", "▉", "█"];
const OPTION_TEXT_LIMIT = 75;

function selectOptionText(text) {
  const characters = [...text];
  if (characters.length <= OPTION_TEXT_LIMIT) return text;
  return `${characters.slice(0, OPTION_TEXT_LIMIT - 1).join("")}…`;
}
...
function generateProgressBar(percentage, size) {
  ...
  return (
    ...
    (Math.floor(blocks) === blocks
      ? ""
      : BAR_BLOCKS[Math.round((blocks - Math.floor(blocks)) * 8)]) +
    BAR_BLOCKS[0].repeat(size - Math.ceil(blocks))
  );
}

function titleSection(poll, { closed } = {}) {
  if (closed) {
    return {
      type: "section",
  ...
  const options = [
    {
      ...
      value: JSON.stringify({ poll: poll.id, action: "edit" }),
    },
    {
      text: { type: "plain_text", text: "Delete" },
      value: JSON.stringify({ poll: poll.id, action: "delete" }),
    },
  ];
...
function choicesSection(poll, choices, votes, names) {
  ...
  for (const [index, choice] of choices.entries()) {
    if (poll.add_choice_setting === "anyone") {
      text += `${names.get(choice.creator_user_id) ?? choice.creator_user_id}: `;
    }
    text += `${choice.text}\n`;

    const voters = votesByChoice.get(choice.id) ?? [];
    if (voters.length && !poll.anonymous) {
      text += voters.map((u) => names.get(u) ?? u).join(", ");
      text += "\n";
    }
...
function voteActions(poll, choices) {
  ...
  for (const choice of choices) {
    options.push({
      text: { type: "plain_text", text: selectOptionText(choice.text) },
...
function contextBlock(poll, names, { closed } = {}) {
  const elements = [
    { type: "mrkdwn", text: `Asked by ${names.get(poll.creator_user_id) ?? poll.creator_user_id}` },
  ];
  ...
  if (!closed && poll.add_choice_setting !== "no_one") {
    elements.push({
      type: "plain_text",
      text:
  ...
  elements.push(
    closed
      ? { type: "mrkdwn", text: "🔒 *Closed*, replaced by a newer anchor" }
      : { type: "plain_text", text: ":pushpin: Anchored to this channel" },
  );

  return { type: "context", elements };
}

export async function buildAnchorPollBlocks(client, poll, choices, votes, { closed = false } = {}) {
  const userIds = [poll.creator_user_id];
  if (poll.add_choice_setting === "anyone") {
    userIds.push(...choices.map((c) => c.creator_user_id));
  }
  if (!poll.anonymous) {
    userIds.push(...votes.map((v) => v.user_id));
  }
  const names = await resolveDisplayNames(client, userIds);

  const blocks = [
    titleSection(poll, { closed }),
    { type: "divider" },
    choicesSection(poll, choices, votes, names),
  ];
  if (!closed) blocks.push(voteActions(poll, choices));
  blocks.push(contextBlock(poll, names, { closed }));
  return blocks;
}
