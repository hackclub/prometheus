...
function closedContext() {
  return {
    type: "context",
    elements: [{ type: "mrkdwn", text: "🔒 *Closed*, replaced by a newer anchor" }],
  };
}
...
function scaleHint() {
  return {
    ...
    elements: [
      {
        type: "mrkdwn",
        text: "1 = Not at all likely · 10 = Extremely likely, reply in this thread to add feedback",
      },
    ],
  };
}

export function buildAnchorNpsBlocks(poll, responses, { closed = false } = {}) {
  const blocks = [titleSection(poll, { closed }), statsSection(poll, responses)];
  return closed ? [...blocks, closedContext()] : [...blocks, ...scoreButtonRows(poll), scaleHint()];
}
