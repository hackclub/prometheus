...
async function reviewMove({ source, dest, kick, exclude, userId, client, logger, deliver, warn }) {
  if (source === dest) return warn("The destination has to be a different channel.");

  if (!(await canMove(client, userId, { source, dest }))) {
    logger.info(`[move] ${userId} denied moving ${source} -> ${dest}`);
    return warn(
      "You need to be a Prometheus manager or Slack channel manager of *both* channels (or a global admin).",
    );
  }
...
async function handleMoveView({ view, body, client, logger }) {
