  ...
  async execute({ command, args, respond, client, context }) {
    ...
    switch (action) {
      case "set": {
        ...
        if (!message) return respond(eph("Usage: `/pro welcome set [ephemeral|dm] <message>`"));
        await setwelcome(ch, message, mode, u);
        try {
