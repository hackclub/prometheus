import { isGlobalAdmin, isChannelPinger, isChannelModerator, isChannelManager } from "../perms.js";
...
const ADMIN_CMDS = [
  ...
  {
    cmd: "/pro channelmanager add @user [pinger|moderator|manager]",
    desc: "Appoint a pinger (pings only), moderator (+ timeouts), or full manager",
  },
...
export default {
  ...
  async execute({ command, respond, context }) {
    ...
    const pinger = admin || isChannelPinger(context.userClient, userId, channelId);
    ...
    if (pinger) {
      ...
      blocks.push({
        type: "section",
        text: { type: "mrkdwn", text: section(":mega: Channel Pings", PING_CMDS) },
      });
    }
