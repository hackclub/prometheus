...
const MANAGER_CMDS = [
  ...
  { cmd: '/pro anchor', desc: 'Create or replace this channel\'s anchor poll' },
  ...
  { cmd: '/pro anchor list [#channel]', desc: 'See the last 5 anchors for a channel, with a details button' },
];
...
const ADMIN_CMDS = [
  ...
  { cmd: '/pro communitysteward add @user [manager]', desc: 'Appoint a moderator (timeout-only) or full manager' },
  { cmd: '/pro communitysteward [remove|list] [@user]', desc: 'Remove or list channel roles' },
];
  ...
  async execute({ command, respond, context }) {
    ...
    if (!moderator) {
      ...
      blocks.push({
        ...
        elements: [{
          type: 'mrkdwn',
          text: '_That\'s all you have here. Need more power? Ask eps to run `/pro communitysteward add @you` in this channel._',
        }],
      });
    }
