...
import { isGlobalAdmin, CHANNEL_ROLES } from "../perms.js";
  ...
  async execute({ command, args, respond, client }) {
    ...
    switch (action) {
      case "add": {
        ...
        if (!id)
          return respond(eph("Usage: `/pro channelmanager add @user [pinger|moderator|manager]`"));
        const role = CHANNEL_ROLES.includes(args[2]) ? args[2] : "moderator";
        addAppointedManager(id, ch, u, role);
        console.log(`[channelmanager] ${u} added ${id} as ${role} in ${ch}`);
        logAdmin(client, {
          action: `added <@${id}> as channel ${role}`,
          adminUser: u,
          channel: ch,
        });
      ...
      case "remove": {
        ...
        logAdmin(client, {
          action: `removed <@${id}> from channel roles`,
          adminUser: u,
          channel: ch,
        });
      ...
      default:
        return respond(
          eph("Usage: `/pro channelmanager [add|remove|list] [@user] [pinger|moderator|manager]`"),
        );
    }
  },
};
