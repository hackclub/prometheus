...
import { joinChannel, closeOldAnchorMessage, parseChannelMention } from '../anchorCommon.js';
...
function buildMessageModalView({ channel, existing, error }) {
  ...
  blocks.push({
    type: 'section',
    text: { type: 'mrkdwn', text: `_You are ${existing ? 'editing' : 'creating'} the anchored message for_ <#${channel}>.` },
  });
  ...
  blocks.push({
    ...
    element: {
      ...
      action_id: 'value',
      ...(existing ? { initial_value: existing.content ? JSON.parse(existing.content) : plainTextToRichText(existing.question) } : {}),
    },
  });
...
export async function openMessageModal(client, triggerId, { channel, existing }) {
  await client.views.open({ trigger_id: triggerId, view: buildMessageModalView({ channel, existing }) });
}
...
async function createAnchorMessageAnchor({ channel, userId, question, content, client, logger }) {
  ...
  try {
    ...
    return { error: `Created the anchor but couldn't post/pin it in <#${channel}>: \`${e.data?.error ?? e.message}\`` };
  }
...
async function handleMessageView({ body, view, client, context, logger }) {
  ...
  if (id) {
    ...
    await logAdmin(client, { action: 'edited the anchor message', adminUser: userId, channel, detail: question });
    return;
  }

  const { error } = await createAnchorMessageAnchor({ channel, userId, question, content, client, logger });
  if (error) {
...
export default {
  ...
  async execute({ command, args, respond, client, context, logger }) {
    ...
    if (action === 'enable' || action === 'disable') {
      ...
      if (!poll) return respond(eph('No anchor configured for this channel yet. Run `/pro anchor` to create one.'));
      ...
      if (poll.type === 'message' && poll.message_ts) {
        try {
          ...
          logger.warn(`anchor message ${action} pin toggle failed in ${channel}: ${e.data?.error ?? e.message}`);
        }
      }
    ...
    if (action === 'status') {
      ...
      if (poll.type === 'message') {
        return respond(eph(`*Anchored message* for <#${channel}>\n${poll.question}\n\n${poll.enabled ? 'Enabled' : 'Disabled'}`));
      }
    ...
    if (action === undefined || (action === 'msg' && rest.length === 0)) {
      ...
      await client.views.open({
        trigger_id: command.trigger_id,
        view: buildMessageModalView({ channel, existing: existing?.type === 'message' ? existing : null }),
      });
      return;
    }
    ...
    if (!text) {
      return respond(eph('Usage: `/pro anchor <message>` to anchor a message, or `/pro anchor`'));
    }

    const { error } = await createAnchorMessageAnchor({ channel, userId, question: text, content: null, client, logger });
