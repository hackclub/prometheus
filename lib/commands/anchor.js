...
async function postAndPinPoll(client, context, poll) {
  const blocks = await buildAnchorPollBlocks(context.userClient, poll, poll.choices, []);
  ...
  await setAnchorPollMessageTs(poll.id, msg.ts);
...
async function createAnchorMessageAnchor({
  channel,
  userId,
  question,
  content,
  ...
  const existing = await getAnchorPoll(channel);
  const poll = await createAnchorMessage(channel, { creator: userId, question, content });
  await closeOldAnchorMessage(client, context, channel, existing, logger);

  if (existing?.type === "nps") {
    try {
      await syncNpsSurvey(await getAnchorPollById(existing.id), client);
    } catch (e) {
...
async function handleCreateView({ body, view, client, context, logger }) {
  ...
  const existing = await getAnchorPoll(channel);
  const poll = await createAnchorPoll(channel, {
    creator: userId,
    question,
    choices,
    anonymous,
  ...
  try {
    await postAndPinPoll(client, context, poll);
    await logAdmin(client, {
...
async function handleEditView({ body, view, client, context, logger }) {
  ...
  const poll = await getAnchorPollById(id);
  ...
  const updated = await updateAnchorPollQuestion(id, question);
  const blocks =
    updated.type === "nps"
      ? buildAnchorNpsBlocks(updated, await getAnchorNpsResponses(id))
      : await buildAnchorPollBlocks(
          context.userClient,
          updated,
          await getAnchorPollChoices(id),
          await getAnchorPollVotes(id),
        );
...
async function handleAddOptionView({ body, view, client, context, logger }) {
  const { id } = JSON.parse(view.private_metadata);
  ...
  await addAnchorPollChoice(id, userId, text);

  const choices = await getAnchorPollChoices(id);
  const votes = await getAnchorPollVotes(id);
...
async function handleMessageView({ body, view, client, context, logger }) {
  ...
  if (id) {
    ...
    const updated = await updateAnchorMessageContent(id, { question, content });
    if (updated.message_ts) {
      try {
        await client.chat.update({
          channel: updated.channel_id,
...
async function historyEntrySummary(poll) {
  ...
  const count =
    poll.type === "nps"
      ? (await getAnchorNpsResponses(poll.id)).length
      : (await getAnchorPollVotes(poll.id)).length;
  return `${header} • ${count} response${count === 1 ? "" : "s"}`;
}

async function historyEntryBlock(poll) {
  return {
    type: "section",
    text: { type: "mrkdwn", text: await historyEntrySummary(poll) },
    accessory: {
      type: "button",
      text: txt("Details"),
...
export default {
  ...
  async execute({ command, args, respond, client, context, logger }) {
    ...
    if (action === "list") {
      ...
      const history = await listAnchorPolls(targetChannel, 5);
      if (!history.length) {
        return respond(eph(`No anchors have been created in <#${targetChannel}> yet.`));
      }

      const entries = await Promise.all(history.map(historyEntryBlock));
      const blocks = [
        {
          type: "section",
          text: {
        ...
        { type: "divider" },
        ...entries.flatMap((entry, i) => (i === 0 ? [entry] : [{ type: "divider" }, entry])),
      ];
    ...
    if (action === "enable" || action === "disable") {
      ...
      const poll = await getAnchorPoll(channel);
      if (!poll)
        return respond(
          eph("No anchor configured for this channel yet. Run `/pro anchor` to create one."),
        );
      await setAnchorPollEnabled(channel, action === "enable");

      if (poll.type === "nps") {
        try {
          await syncNpsSurvey(await getAnchorPollById(poll.id), client);
        } catch (e) {
    ...
    if (action === "status") {
      ...
      if (poll.type === "nps") {
        const responses = await getAnchorNpsResponses(poll.id);
      ...
      const choices = await getAnchorPollChoices(poll.id);
      const votes = await getAnchorPollVotes(poll.id);
    ...
    if (action === "poll") {
      ...
      const existingChoices =
        existing?.type === "poll" ? await getAnchorPollChoices(existing.id) : [];
