...
export default async function postingGateListener({ event, client, context, logger }) {
  ...
  if (await isExternalUser(context.userClient, message.user, message.user_team)) return;
  ...
  if (gateKicksOnJoin(gate)) {
    let kicked = false;
    try {
      kicked = await kickForPostingGate(context.userClient, message.channel, message.user);
    } catch (error) {
      logger.warn(`posting gate kick failed: ${error.data?.error ?? error.message}`);
    }

    if (kicked) {
      try {
        await postPostingGateDm(client, message.user, gate, { retry: true, kicked: true });
      } catch (error) {
        logger.warn(`posting gate kick dm failed: ${error.data?.error ?? error.message}`);
      }
      return false;
    }
  }
