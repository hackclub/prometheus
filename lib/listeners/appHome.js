...
async function buildHome(section = "overview", page = 0) {
  return buildHomeBlocks(
    {
      admins: await listGlobalAdmins(),
      managers: await listAllAppointedManagers(),
      bans: await listAllChannelBans(),
    },
    section,
    page,
  );
...
export default async function ({ event: ev, client }) {
  ...
  console.log(`[home] app home opened by ${ev.user} (admin: ${await isGlobalAdmin(ev.user)})`);
  const blocks = (await isGlobalAdmin(ev.user)) ? await buildHome() : normie;
  try {
...
async function refreshHome(client, userId, section = "overview", page = 0) {
  ...
  await client.views.publish({
    user_id: userId,
    view: { type: "home", blocks: await buildHome(section, page) },
  });
}
...
export const actions = [
  {
    ...
    async execute({ action, body, client }) {
      if (!(await isGlobalAdmin(body.user.id))) return;
  ...
  },
  {
    ...
    async execute({ action, body, client }) {
      ...
      await removeGlobalAdmin(target);
      logAdmin(client, {
  ...
  },
  {
    ...
    async execute({ action, body, client }) {
      ...
      await removeAppointedManager(uid, ch);
      logAdmin(client, {
...
export const views = [
  {
    ...
    async handleView({ view, body, client }) {
      ...
      await addGlobalAdmin(target, invoker);
      logAdmin(client, {
  ...
  },
  {
    ...
    async handleView({ view, body, client }) {
      ...
      await addAppointedManager(uid, ch, invoker, role);
      logAdmin(client, {
