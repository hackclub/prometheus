...
const ROLE_EMOJI = { manager: ":baldfols:", moderator: ":bonk:", pinger: ":mega:" };
...
function channelRoleBlocks(managers, requestedPage) {
  ...
  if (!items.length) {
    ...
    for (const manager of items) {
      ...
      const emoji = ROLE_EMOJI[manager.role] ?? ":bonk:";
      blocks.push({
        type: "section",
        text: {
          type: "mrkdwn",
          ...
          blocks: [
            ...
            },
            {
              ...
              element: {
                ...
                options: [
                  { text: { type: "plain_text", text: "Pinger (pings only)" }, value: "pinger" },
