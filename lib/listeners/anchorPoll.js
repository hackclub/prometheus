import { getAnchorPoll, getAnchorPollChoices, getAnchorPollVotes, getAnchorNpsResponses, recordNpsComment, setAnchorPollMessageTs } from '../db.js';
...
async function repost(client, logger, poll) {
  if (poll.message_ts) {
    try {
      ...
      logger.warn(`anchor poll repost-delete failed in ${poll.channel_id}: ${e.data?.error ?? e.message}`);
    }
  }
...
async function captureNpsThreadFeedback({ client, context, logger, channel, poll, event }) {
  ...
  try {
    await syncNpsResponse(poll, userId, { comment: text }, client);
  } catch (e) {
  ...
  if (threadTs === poll.message_ts) {
    if (poll.type === 'nps') {
      ...
      try {
        ...
        logger.warn(`anchor poll thread redirect failed in ${channel}: ${e.data?.error ?? e.message}`);
      }
    }
  }
