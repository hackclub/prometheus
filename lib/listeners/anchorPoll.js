...
async function repost(client, context, logger, poll) {
  if (poll.message_ts) {
    try {
  ...
  const blocks =
    poll.type === "nps"
      ? buildAnchorNpsBlocks(poll, await getAnchorNpsResponses(poll.id))
      : poll.type === "message"
        ? buildAnchorMessageBlocks(poll)
        : await buildAnchorPollBlocks(
            context.userClient,
            poll,
            await getAnchorPollChoices(poll.id),
            await getAnchorPollVotes(poll.id),
          );
...
async function captureNpsThreadFeedback({ client, context, logger, channel, poll, event }) {
  ...
  await recordNpsComment(poll.id, userId, text);
  ...
  if (poll.message_ts) {
    try {
      await client.chat.update({
        ...
        ts: poll.message_ts,
        text: poll.question,
        blocks: buildAnchorNpsBlocks(poll, await getAnchorNpsResponses(poll.id)),
      });
    } catch (e) {
...
export default async function anchorPollListener({ event, client, context, logger }) {
  ...
  await repost(client, context, logger, poll);
}
