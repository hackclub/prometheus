...
export async function deleteAnchor(client, poll, userId, logger) {
  await deleteAnchorPoll(poll.id);
...
export async function closeOldAnchorMessage(client, context, channel, oldPoll, logger) {
  ...
  try {
    const blocks =
      oldPoll.type === "nps"
        ? buildAnchorNpsBlocks(oldPoll, await getAnchorNpsResponses(oldPoll.id), { closed: true })
        : await buildAnchorPollBlocks(
            context.userClient,
            oldPoll,
            await getAnchorPollChoices(oldPoll.id),
            await getAnchorPollVotes(oldPoll.id),
            { closed: true },
          );
