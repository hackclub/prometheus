...
export async function joinChannel(client, channel) {
  try {
    ...
    if (error === 'method_not_supported_for_channel_type') {
      ...
      } catch { /* fall through to the DM/MPIM message below */ }
  ...
  try {
    const blocks =
      oldPoll.type === 'nps'
        ...
        : buildAnchorPollBlocks(oldPoll, getAnchorPollChoices(oldPoll.id), getAnchorPollVotes(oldPoll.id), { closed: true });
